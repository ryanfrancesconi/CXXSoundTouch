...
void FIFOSampleBuffer::ensureCapacity(uint capacityRequirement) {
    ...
    if (capacityRequirement > getCapacity()) {
        ...
        sizeInBytes =
            (capacityRequirement * channels * sizeof(SAMPLETYPE) + 4095) &
            (uint)-4096;
        ...
        // Align the buffer to begin at 16byte cache line boundary for optimal
        // performance
        temp = (SAMPLETYPE *)SOUNDTOUCH_ALIGN_POINTER_16(tempUnaligned);
        if (samplesInBuffer) {
            memcpy(temp, ptrBegin(),
...
uint FIFOSampleBuffer::numSamples() const { return samplesInBuffer; }
...
int FIFOSampleBuffer::isEmpty() const { return (samplesInBuffer == 0) ? 1 : 0; }
