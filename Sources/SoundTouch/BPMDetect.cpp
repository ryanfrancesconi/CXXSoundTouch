...
static void _SaveDebugData(const char *name, const float *data, int minpos,
                           int maxpos, double coeff) {
...
#else // ifdef _CREATE_BPM_DEBUG_FILE
#define _SaveDebugData(name, a, b, c, d)
#define _SaveDebugBeatPos(name, b)
#endif // ifdef _CREATE_BPM_DEBUG_FILE
...
BPMDetect::BPMDetect(int numChannels, int aSampleRate) : beat_lpf(_LPF_coeffs) {
    ...
    if ((decimateBy <= 0) ||
        (decimateBy * DECIMATED_BLOCK_SIZE < INPUT_BLOCK_SIZE)) {
...
///
/// Anti-alias filtering is done simply by averaging the samples. This is really
/// a poor-man's anti-alias filtering, but it's not so critical in this kind of
/// application (it'd also be difficult to design a high-quality filter with
/// steep cut-off at very narrow band)
int BPMDetect::decimate(SAMPLETYPE *dest, const SAMPLETYPE *src,
                        int numsamples) {
    int count, outcount;
    LONG_SAMPLETYPE out;
...
float BPMDetect::getBpm() {
    ...
    _SaveDebugData("soundtouch-bpm-xcorr.txt", xcorr, windowStart, windowLen,
                   coeff);
    ...
    // save bpm debug data if debug data writing enabled
    _SaveDebugData("soundtouch-bpm-smoothed.txt", data, windowStart, windowLen,
                   coeff);
