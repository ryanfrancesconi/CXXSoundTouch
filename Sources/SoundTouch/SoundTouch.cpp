...
SoundTouch::SoundTouch() {
    ...
    virtualPitch = virtualRate = virtualTempo = 1.0;
...
uint SoundTouch::getVersionId() { return SOUNDTOUCH_VERSION_ID; }
...
void SoundTouch::calcEffectiveRateAndTempo() {
    ...
    if (!TEST_FLOAT_EQUAL(rate, oldRate))
        pRateTransposer->setRate(rate);
    if (!TEST_FLOAT_EQUAL(tempo, oldTempo))
...
void SoundTouch::putSamples(const SAMPLETYPE *samples, uint nSamples) {
    ...
    // accumulate how many samples are expected out from processing, given the
    // current processing setting
    samplesExpectedOut += (double)nSamples / ((double)rate * (double)tempo);

#ifndef SOUNDTOUCH_PREVENT_CLICK_AT_RATE_CROSSOVER
    if (rate <= 1.0f) {
        // transpose the rate down, output the transposed sound to tempo changer
        // buffer
        assert(output == pTDStretch);
...
bool SoundTouch::setSetting(int settingId, int value) {
    ...
    pTDStretch->getParameters(&sampleRate, &sequenceMs, &seekWindowMs,
                              &overlapMs);
...
/// to output buffer and removes them from the sample buffer. If there are less
/// than 'numsample' samples in the buffer, returns all that available.
///
...
///
/// Used to reduce the number of samples in the buffer when accessing the sample
...
double SoundTouch::getInputOutputSampleRatio() { return 1.0 / (tempo * rate); }
