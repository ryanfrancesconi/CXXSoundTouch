...
#ifndef SOUNDTOUCH_PREVENT_CLICK_AT_RATE_CROSSOVER
    // Disable Anti-alias filter if desirable to avoid click at rate change zero
    // value crossover
...
bool RateTransposer::isAAFilterEnabled() const { return bUseAAFilter; }

AAFilter *RateTransposer::getAAFilter() { return pAAFilter; }
...
void RateTransposer::processSamples(const SAMPLETYPE *src, uint nSamples) {
    ...
    if (pTransposer->rate < 1.0f) {
        ...
        // folding over the lover frequencies), then transpose.
...
TransposerBase *TransposerBase::newInstance() {
    ...
    // Notice: For integer arithmetic support only linear algorithm (due to
    // simplest calculus)
